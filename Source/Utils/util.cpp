...
vector<string> Utils::get_filelist(string foldname)
{
	...
	while(FindNextFile(file, &fileData))
	{
		...
		for (int i = 0; i < 5; i ++)       //ÅÅ³ý·ÇÍ¼ÏñÎÄ¼þ
		{
			...
			{
				if (lineStr.find("NISwGSP") == string::npos) {
	...
	if (image.channels() == 4)
	{
		for (int i = 0; i < rows; i ++)
		{
			for (int j = 0; j < cols; j ++)
			{
				...
				{
					int gray = 0.299 * imgPtr[4 * (i * cols + j) + 2] + 0.587 * imgPtr[4 * (i * cols + j) + 1] + 0.114 * imgPtr[4 * (i * cols + j) + 0];
					if(gray<10)
					{
						continue;
					}
...
{
	vector<int> iset1(begin(roiIndexs1),end(roiIndexs1));  
	vector<int> iset2(begin(roiIndexs2),end(roiIndexs2));
