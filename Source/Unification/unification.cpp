...
void ToneUnifier::initialization()
{
	...
	string paramPath = "./input/ParamsPro.txt";
	ifstream fin;
	...
	for (int i = 0; i < _imgNum; i ++)
	{
		...
		resize(image,image,Size(2000,1000));
...
void ToneUnifier::buildCorrespondings()
{
	...
	for (int i = 0; i < _imgNum; i ++)
	{
		...
		resize(curImage, curImage, Size(2000, 1000));
		...
		for (int j = 0; j < curNeigbList.size(); j ++)
		{
			...
			resize(adjImage, adjImage, Size(2000, 1000));
			//! covert to Lab color space
			...
			if (0)
			{
				...
				string path = Utils::baseDir + to_string(i) + "_"+ to_string(adjNo) + "overlap.png";
				cout << path;
...
void ToneUnifier::applyColorRemappingforImages(bool needIndividuals, bool applyRemapping)
{
	...
	for (int i = 0; i < _imgNum; i ++)
	{
		...
		Mat curImagef2 = ColorSpace::RGB2YCbCr(curImage, roiIndexList);
		...
		double* oriMap = (double*)curImagef2.data;
		Mat warpMap;
		if (needIndividuals)
		{
			warpMap = Mat(rows, cols, CV_8UC4, Scalar(0,0,0,0));
		}
		...
		for (int j = 0; j < roiIndexList.size(); j ++)
		{
			...
			//original YCbCr to RGB
			double Y2 = oriMap[3 * index + 0];        //! Y
			double Cb2 = oriMap[3 * index + 1] - 127.5;   //! Cb-128
			double Cr2= oriMap[3 * index + 2] - 127.5;   //! Cr-128
			int R2 = int(Y2 + 1.402 * Cr2);
			int G2 = int(Y2 - 0.3441 * Cb2 - 0.7141 * Cr2);
			int B2 = int(Y2 + 1.7720 * Cb2);
			int Bi2 = min(255, max(0, B2));
			int Gi2 = min(255, max(0, G2));
			int Ri2 = min(255, max(0, R2));
			...
			if (needIndividuals)
			{
				...
				float weight = 1.0;
				if (curIndex == 0||curIndex== _imgNum - 1) {
					int y = index / cols;
					int x = index % cols;

					if (curIndex ==0 && x < 100) {
						weight = (float)x / 100.0;	
					}
					else if(curIndex == _imgNum - 1 && x > cols - 100) {
						weight = 1 - (float)(x-(cols-100)) / 100.0;
					}
					else {
						weight = 1;
					}
				...
				warpPtr[4 * index + 0] = (float)Bi * weight + Bi2 * (1 - weight);
				warpPtr[4 * index + 1] = (float)Gi * weight + Gi2 * (1 - weight);
				warpPtr[4 * index + 2] = (float)Ri * weight + Ri2 * (1 - weight);
			}
		}
		if (needIndividuals)
		{
			...
			string savePath = result_dir + string(name);
			cout << savePath << endl;
...
void ToneUnifier::outputEvaluationReport()
{
	...
	for (int i = 0; i < _imgNum; i ++)
	{
		...
		//resize(srcImage, srcImage, Size(2000, 1000));
		Mat dstImage = imread(_savePathList[i], 0);
		//resize(dstImage, dstImage, Size(2000, 1000));
...
void ToneUnifier::destroy() {
	cout << "destroy" << endl;
	_imageInforList.clear();
	_savePathList.clear();
	_filePathList.clear();
	_fixedImgList.clear();
}
